...
for i in range(3):
    for j in range(i, 3):
        ...
        axis[1+i][j].scatter(train_dataset.inputs[:, 0], train_dataset.inputs[:, 1],
        ...
        if i != j:
            ...
            axis[1+j][i].scatter(train_dataset.inputs[:, 0], train_dataset.inputs[:, 1],
                        c = plt.get_cmap('tab10')(train_dataset.targets.numpy().astype(np.int32)), 
                        alpha = 0.2, label = "Training Dataset")
...
samples = la.predictive_samples(torch.tensor(positions, dtype = args.dtype), n_samples = 10000)
mean = torch.mean(samples, 0)
lla_var = torch.einsum("sna, snb -> nab", samples - mean, samples - mean)/(samples.shape[0] - 1)
