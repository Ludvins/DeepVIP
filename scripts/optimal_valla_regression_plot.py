...
_, valla_prior_var = sparseLA.forward_prior(torch.tensor(
    test_dataset.inputs,
    device = args.device,
    dtype = args.dtype))
...
axis[0].plot(test_dataset.inputs.flatten()[sort], valla_mean.flatten()[sort], label = "Predictions", color = "black")
