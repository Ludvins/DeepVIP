...
train_dataset_cv, validation_dataset_cv = torch.utils.data.random_split(train_dataset, 
                                                                  [50000, 10000],
                                                                  generator
                                                                  )
...
for prior_std in prior_stds:
    ...
    ella = ELLA_Optimized(create_ad_hoc_mlp(f),
                f.output_size,
                args.num_inducing,
    ...
    ella.fit_loader(torch.tensor(train_dataset.inputs, device = args.device, dtype = args.dtype),
            torch.tensor(train_dataset.targets, device = args.device, dtype = args.dtype),
            train_loader)

    metrics = score(ella, val_loader, SoftmaxClassification, use_tqdm=True, device = args.device, dtype = args.dtype)
...
test_metrics = score(ella, val_loader, SoftmaxClassification, use_tqdm=True, device = args.device, dtype = args.dtype)
...
df.to_csv("results/ELLA_{}_MAP_iterations={}_prior_std={}_K={}_M={}.csv".format(args.dataset_name, str(args.MAP_iterations), str(args.prior_std), str(ella.K), str(ella.M)), index = False)
