...
mlp_model = get_mlp(input_dim, output_dim, [50, 50], torch.nn.Tanh, device = "cpu", dtype = torch.float64)
...
x, Kx, Kxz, Kzz = mlp_model_adhoc.get_full_kernels(X, X, torch.zeros(X.shape[0], dtype = torch.int64))

print("Testing both models have the same kernel diagonal: ", end = "")
print(test_equal(
    torch.diagonal(ad_kernel,dim1 = 0, dim2 = 1).permute(2,0,1),
    Kx
    ))
...
print("Testing both models have the same kernel diagonal: ", end = "")
print(test_equal(
    torch.diagonal(ad_kernel_zz,dim1 = -2, dim2 = -1),
    Kzz
    ))