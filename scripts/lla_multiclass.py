...
f = get_mlp(train_dataset.inputs.shape[1], args.dataset.output_dim, [50, 50], torch.nn.Tanh,
...
str_name = "weights/multiclass_weights_"+args.dataset_name+str(args.weight_decay)
try:
...
train_metrics = score(lla, train_loader, SoftmaxClassification, use_tqdm=True, device = args.device, dtype = args.dtype)

df = pd.DataFrame.from_dict(train_metrics, orient="index").transpose()
print(df)
test_metrics = score(lla, test_loader, SoftmaxClassification, use_tqdm=True, device = args.device, dtype = args.dtype)
...
df.to_csv("results/LLA_{}_MAP_iterations={}_prior_std={}.csv".format(args.dataset_name, str(args.MAP_iterations), str(args.prior_std)), index = False)
