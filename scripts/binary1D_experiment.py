...
def plot_model(model, index, name):
    ...
    axis[index][0].plot(test_dataset.inputs.flatten()[sort], mean.flatten()[sort], label = "Predictive mean",
                color = "teal")

    mean, var = model(torch.tensor(test_dataset.inputs, device = args.device, dtype = args.dtype))
    ...
    axis[index][1].plot(test_dataset.inputs.flatten()[sort], mean.flatten()[sort], label = "Predictive mean",
                color = "teal")
    axis[index][1].fill_between(test_dataset.inputs.flatten()[sort],
...
KL_ELLA = - np.log(std_lla) + np.log(std_ella) - 0.5 + ((std_lla**2)/(2*(std_ella**2)))
KL_ELLA = np.sum(KL_ELLA)
KL_VaLLA = - np.log(std_lla) + np.log(std_valla) - 0.5 + ((std_lla**2)/(2*(std_valla**2)))
