...
from src.sparseLA import VaLLA_Optimized, GPLLA
...
lla = GPLLA(f, 
            prior_std = args.prior_std,
            likelihood_hessian=lambda x,y: torch.ones_like(y).unsqueeze(-1).permute(1, 2, 0) / ll_var,
            likelihood=Gaussian(device=args.device, 
                        log_variance=np.log(ll_var),
                        dtype = args.dtype), 
            backend = BackPackInterface(f, f.output_size),
...
axis[0].plot(full_dataset.inputs.flatten()[sort], valla_mean.flatten()[sort], label = "Predictions", color = "black")
axis[0].fill_between(full_dataset.inputs.flatten()[sort],
...
axis[1].fill_between(full_dataset.inputs.flatten()[sort],
...
sort = np.argsort(full_dataset.inputs.flatten())
axis[0].plot(full_dataset.inputs.flatten()[sort],
...
save_str = "VaLLA_dataset={}_M={}".format(args.dataset_name,args.num_inducing, args.seed)
...
KL1 = - np.log(lla_std) + np.log(valla_std) - 0.5 + ((lla_std**2)/(2*valla_std**2))
KL1 = np.sum(KL1)

KL2 = - np.log(valla_std) + np.log(lla_std) - 0.5 + ((valla_std**2)/(2*lla_std**2))
KL2 = np.sum(KL2)
...
test_metrics = score(sparseLA, test_loader, MetricsRegression, use_tqdm=True, device = args.device, dtype = args.dtype, ll_var = ll_var)
