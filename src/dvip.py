...
class DVIP_Base(tf.keras.Model):
    def __init__(self, likelihood, layers, num_data, num_samples=1, y_mean=0.0, 
                 y_std=1.0, dtype = tf.float64, **kwargs):
        """ 
        ...
        **kwargs : dict, optional
                   ...
                   Refer to tf.keras.Model for more information.
        """
        super().__init__(name="DVIP_Base", dtype=dtype)
    ...
    def predict_f(self, predict_at, num_samples, full_cov=False):
        _, Fmeans, Fvars = self.propagate(predict_at, full_cov=full_cov,
    ...
    def nelbo(self, inputs, outputs):
        ...
        # Compute KL term
        KL = tf.reduce_sum([layer.KL() for layer in self.layers_dgp])
