...
class ELLA_Base(torch.nn.Module):
    ...
    def predict_f(self, X):
        ...
        with torch.no_grad():
            ...
            prior = torch.eye(self.K, device = self.devive, dtype=self.dtype) / (self.prior_std**2)
    ...
    def fit_loader_val(self, X_train, y_train, train_loader, val_loader, val_steps=1, verbose = True):
        ...
        with torch.no_grad():
            ...
            for i in iters:
                ...
                self.G += torch.einsum("nak, abn, nbg -> kg", phi_train, Lambda, phi_train)
        ...
        with torch.no_grad():
            ...
            if self.output_size > 1:
                phi_z = self.backpack.jacobians_on_outputs(
                    Xz, y_train[indexes].to(torch.long),
                    enable_back_prop = False
                )
            else:
                phi_z = self.backpack.jacobians(Xz,
                    enable_back_prop = False)
