...
sparseLA = VaLLA(
    ...
    output_dim=1,
    backend=BackPackInterface(f[:-1], args.dataset.output_dim),
...
mean, _ = sparseLA.predict_mean_and_var(
    torch.tensor(full_dataset.inputs, dtype=args.dtype, device=args.device)
)
pred = 1.0 * (mean >= 0.5)
...
cp = axis[3][0].contourf(X, Y, valla_std, cmap=cm.gray)
...
plt.savefig(
    "VaLLA_{}=M={}_prior={}.pdf".format(
        args.dataset_name, args.num_inducing, str(args.prior_std)
    ),
    format="pdf",
)
