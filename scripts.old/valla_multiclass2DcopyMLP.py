...
from utils.models import get_mlp, MLP, create_ad_hoc_mlp
...
sparseLA = VaLLAMultiClassSubsetOptimized(
    ...
    likelihood=GaussianMultiClassSubset(device=args.device, dtype=args.dtype),
...
axis[0][0].scatter(
    ...
    c=color_map(train_dataset.targets.astype(np.int32)),
    alpha=0.8,
...
for i in np.arange(
    start=0,
    stop=inducing_history.shape[0],
    step=np.max([inducing_history.shape[0] // 100, 1]),
):
    axis[0][2].scatter(
        inducing_history[i, :, 0],
        inducing_history[i, :, 1],
        c=colors,
        s=sizes[i],
        alpha=0.8,
    )
...
for i in range(3):
    for j in range(i, 3):
        cp = axis[1 + i][j].contourf(
            X, Y, valla_var[:, i, j].reshape(n_samples, n_samples), cmap=color_map
        )
        ...
        if i != j:
            cp = axis[1 + j][i].contourf(
                X, Y, valla_var[:, j, i].reshape(n_samples, n_samples), cmap=color_map
            )
...
plt.savefig(
    "VaLLA_{}_M={}_prior={}.pdf".format(
        args.dataset_name, args.num_inducing, args.prior_std
    ),
    bbox_inches="tight",
)
# plt.show()
...
df.to_csv(
    path_or_buf="results/SubVaLLA_{}_dataset={}_M={}_prior={}_seed={}.csv".format(
