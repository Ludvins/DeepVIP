...
try:
    ...
    torch.save(f.state_dict(), "weights/multiclass_weights_blobs")
...
axis[0][0].scatter(
    ...
    c=color_map(train_dataset.targets.numpy().astype(np.int32)),
    alpha=0.8,
...
map_pred_pos = (
    f(torch.tensor(positions, device=args.device, dtype=args.dtype))
    .detach()
    .cpu()
    .numpy()
    .reshape(n_samples, n_samples, 3)
)

axis[0][1].contourf(
    X, Y, sigmoid(map_pred_pos[:, :, 0]), cmap=plt.get_cmap("Blues"), alpha=0.33
)
axis[0][1].contourf(
    X, Y, sigmoid(map_pred_pos[:, :, 1]), cmap=plt.get_cmap("Oranges"), alpha=0.33
)
axis[0][1].contourf(
    X, Y, sigmoid(map_pred_pos[:, :, 2]), cmap=plt.get_cmap("Greens"), alpha=0.33
)

map_pred = (
    f(torch.tensor(train_dataset.inputs, device=args.device, dtype=args.dtype))
    .detach()
    .cpu()
    .numpy()
)
...
axis[0][1].scatter(
    train_dataset.inputs[:, 0],
    train_dataset.inputs[:, 1],
    c=color_map(map_pred.astype(np.int32)),
    alpha=0.8,
    label="Training Dataset",
)
...
f_mu, lla_var = la._glm_predictive_distribution(
    torch.tensor(positions, dtype=args.dtype)
)
...
for i in range(3):
    for j in range(i, 3):
        cp = axis[1 + i][j].contourf(
            X, Y, lla_var[:, i, j].reshape(n_samples, n_samples), cmap=color_map
        )
        divider = make_axes_locatable(axis[1 + i][j])
        ...
        axis[1 + i][j].scatter(
            train_dataset.inputs[:, 0],
            train_dataset.inputs[:, 1],
            c=plt.get_cmap("tab10")(train_dataset.targets.numpy().astype(np.int32)),
            alpha=0.2,
            label="Training Dataset",
        )

        if i != j:
            cp = axis[1 + j][i].contourf(
                X, Y, lla_var[:, j, i].reshape(n_samples, n_samples), cmap=color_map
            )
...
plt.savefig("LLA_laplace_latent_{}.pdf".format(args.dataset_name), bbox_inches="tight")
...
samples = la.predictive_samples(
    torch.tensor(positions, dtype=args.dtype), n_samples=10000
)
mean = torch.mean(samples, 0)
lla_var = torch.einsum("sna, snb -> nab", samples - mean, samples - mean) / (
    samples.shape[0] - 1
)
...
plt.savefig("LLA_laplace_{}.pdf".format(args.dataset_name), bbox_inches="tight")
