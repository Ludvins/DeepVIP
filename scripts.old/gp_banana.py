...
mean, var = lla.predict_mean_and_var(
    torch.tensor(positions, dtype=torch.float64, device=args.device)
)
...
map_pred = (
    f[:-1](torch.tensor(X, device=args.device, dtype=args.dtype)).detach().cpu().numpy()
)
map_pred = 1.0 * (sigmoid(map_pred) >= 0.5)


axis[0][1].scatter(
    X[:, 0], X[:, 1], c=color_map(map_pred), alpha=0.8, label="Predictions"
)
axis[0][1].set_title("MAP predictions")


mean, _ = lla.predict_mean_and_var(
    torch.tensor(X, dtype=args.dtype, device=args.device)
)
